...
import {
  GlobeIcon,
  GearIcon,
  ArrowSquareOutIcon, AdcubumIcon,
...
export default function Adcubum() {
  ...
  const webIndexingStatuses: ConnectorIndexingStatus<WebConfig, {}>[] =
    connectorIndexingStatuses?.filter(
      (connectorIndexingStatus) =>
        connectorIndexingStatus.connector.source === "adcubum"
    ) ?? [];
  ...
  return (
    <div className="mx-auto container">
      ...
      <AdminPageTitle icon={<AdcubumIcon size={32} />} title="Adcubum docs" />
      ...
      <Card className="dark:bg-neutral-700">
        <ConnectorForm<WebConfig>
          nameBuilder={(values) => `AdcubumConnector-${values.base_url}`}
          ...
          validationSchema={Yup.object().shape({
            base_url: Yup.string().required(
              "Please enter the website URL to scrape e.g. https://main-adcubum-syrius-documentation.apps.corp.clusters.adcubum.com/en-GB/index.html"
            ),
